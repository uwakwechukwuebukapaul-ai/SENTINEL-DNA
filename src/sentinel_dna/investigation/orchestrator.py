...
class InvestigationOrchestrator:
    ...
    def run(self, context: InvestigationContext) -> InvestigationResult:
        tasks = self.planner.create_plan(context, self)
        self.runtime.execute(context, tasks)
        return InvestigationResult(
            plan_name=self.planner.plan_name,
            results=self._assemble_results(context),
    ...
    def load_context(self, context: InvestigationContext) -> None:
        ...
        description = str(alert.get("description") or alert.get("body") or "Alert submitted for investigation.")
        case = Case(title=title, description=description, severity=severity, case_id=context.case_id)
        case.add_event("case_created", "Case created")
        case.add_event("investigation_triggered", "Investigation triggered", {"alert": alert})
    ...
    def collect_evidence(self, context: InvestigationContext) -> None:
        evidence = self.evidence_engine.normalize_email(
            {
                ...
                "subject": context.alert.get("subject", context.alert.get("title", "No subject")),
                "body": context.alert.get("body", context.alert.get("description", "")),
            }
        )
        ...
        if not context.iocs:
            context.uncertainties.append("No IOCs were discovered in the submitted alert evidence.")
    ...
    def enrich_iocs(self, context: InvestigationContext) -> None:
        ...
        for ioc in context.iocs:
            enriched[ioc] = {
                "indicator": ioc,
                "type": "url" if ioc.lower().startswith(("http://", "https://")) else "email_or_domain",
                "source": "local_rules",
                "reputation": "suspicious" if any(term in ioc.lower() for term in ["login", "verify", "password"]) else "unknown",
            }
    ...
    def correlate_entities(self, context: InvestigationContext) -> None:
        ...
        if not correlations:
            context.uncertainties.append("No event or entity correlations could be established from available evidence.")
    ...
    def build_timeline(self, context: InvestigationContext) -> None:
        context.timeline = [
            ...
            for evidence in sorted(context.evidence_items, key=lambda item: item.observed_at)
        ]

    def evaluate_threat_intelligence(self, context: InvestigationContext) -> None:
        suspicious_iocs = [
            ioc for ioc, intel in context.intelligence.get("iocs", {}).items() if intel["reputation"] == "suspicious"
        ]
        context.intelligence["threat"] = {
            "source": "local_rules",
            "suspicious_iocs": suspicious_iocs,
            "summary": "Local indicator rules found suspicious IOCs." if suspicious_iocs else "No known suspicious IOCs found locally.",
        }

    def map_mitre_attack(self, context: InvestigationContext) -> None:
        text = self._evidence_text(context).lower()
        mappings = []
        if any(term in text for term in ["password", "credential", "mfa", "verify", "login"]):
            mappings.append({"technique_id": "T1566", "technique": "Phishing", "evidence": "Credential-themed email content"})
        if any(term in text for term in ["invoice", "wire"]):
            mappings.append({"technique_id": "T1657", "technique": "Financial Theft", "evidence": "Payment-themed alert content"})
        context.mitre_attack = mappings
        if not mappings:
            context.uncertainties.append("No MITRE ATT&CK technique could be mapped from current evidence.")
    ...
    def calculate_confidence(self, context: InvestigationContext) -> None:
        if not context.evidence_items:
            ...
            score = sum(evidence.confidence for evidence in context.evidence_items) / len(context.evidence_items)
            if context.errors:
                score -= 0.15
        ...
        context.confidence = {
            ...
            "basis": "Derived from evidence confidence, task errors, and unresolved uncertainty.",
    ...
    def generate_decision_intelligence(self, context: InvestigationContext) -> None:
        risk_level = context.risk.level if context.risk is not None else "unknown"
        context.decision_intelligence = {
            "risk_level": risk_level,
            "confidence_score": context.confidence.get("score", 0.0),
            "recommended_decision": "escalate" if risk_level in {"critical", "high"} else "monitor",
            "rationale": context.reasoning.get("conclusion", "Insufficient evidence for a conclusion."),
        }

    def produce_recommendations(self, context: InvestigationContext) -> None:
        risk_level = context.risk.level if context.risk is not None else "low"
        context.recommendations = self.investigation_engine._actions_for_risk(risk_level)

    def generate_report(self, context: InvestigationContext) -> None:
        if context.case is None or context.risk is None:
            context.report = {"status": "incomplete", "reason": "Missing case or risk assessment."}
            return
        summary = self.investigation_engine.summarize(context.case, context.evidence_items, context.risk)
        context.report = {
    ...
    def _assemble_results(self, context: InvestigationContext) -> dict[str, Any]:
        ...
        return {
            ...
            "plan": {
                ...
                "tasks": [task.name for task in self.planner.create_plan(context, self)],
            },
            "tasks": context.task_results,
            "evidence": [vars(evidence) for evidence in context.evidence_items],
            "iocs": context.iocs,
    ...
    def _validated_alert(self, alert: dict[str, Any]) -> dict[str, Any]:
        if not isinstance(alert, dict):
            raise TypeError("alert must be a dictionary")
        return {str(key): value for key, value in alert.items()}

    def _evidence_text(self, context: InvestigationContext) -> str:
        return " ".join(f"{evidence.summary} {evidence.raw}" for evidence in context.evidence_items)

    def _conclusion(self, context: InvestigationContext) -> str:
        risk_level = context.risk.level if context.risk is not None else "unknown"
        classification = context.threat_classification.get("classification", "unknown")
        if not context.evidence_items:
            return "No evidence was collected; the investigation remains uncertain."
        return f"Available evidence supports a {classification} classification with {risk_level} risk."
