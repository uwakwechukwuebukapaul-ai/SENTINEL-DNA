...
<body>
  ...
  {% for item in investigations %}
    <section class="card">
      ...
      <p class="risk">Risk: {{ item.risk.level|upper }} {{ item.risk.score }}/100</p>
      <p>{{ item.summary.executive_summary }}</p>
      <h3>Recommended Actions</h3>
      <ul>{% for action in item.summary.recommended_actions %}<li>{{ action }}</li>{% endfor %}</ul>
    </section>
  {% else %}
    <section class="card">No cases yet. Run the CLI demo to create a sample investigation.</section>
  {% endfor %}
</body>
</html>
"""
...
def create_app(data_dir: str | None = None) -> Flask:
    app = Flask(__name__)
    resolved_data_dir = data_dir or os.getenv("SENTINEL_DNA_DATA_DIR", "data")
    ...
    def index():
        ...
        for case in case_store.list_cases():
            evidence_items = [evidence_engine.get(evidence_id) for evidence_id in case.evidence_ids]
            risk = risk_engine.assess(evidence_items)
            summary = investigation_engine.summarize(case, evidence_items, risk)
            investigations.append({"case": case, "risk": risk, "summary": summary})
        return render_template_string(PAGE, investigations=investigations)
