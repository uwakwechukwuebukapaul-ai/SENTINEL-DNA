...
def _artifact(
    *, tenant_id: str, username: str, email: str, target_identity: str,
    requester_actor_id: str, approval_transaction_id: str, expires_at: str | datetime,
) -> dict[str, str]:
    ...
    *, db, username: str, email: str, tenant_id: str,
    ...
    try:
        with db.session() as connection:
            ...
            authorization = StagingBootstrapAuthorizationService(
                db, authority=authority, audit=audit
            )
            pending = authorization.get(approval_id, connection=connection)
            artifact = canonical_artifact(
                operation=BOOTSTRAP_OPERATION,
                purpose=PURPOSE,
                environment="staging",
                control_tenant_id=configuration.control_tenant_id,
                target_username=username,
                target_email=email,
                target_role=ROLE,
                database_target_identity=configuration.database_target_identity,
                requester_actor_id=pending.requester_actor_id,
                approval_transaction_id=approval_transaction_id,
            ...
            result = service.provision(
                ...
                audit_actor_id=pending.reviewer_actor_id or "",
            )
            ...
            audit.record(
                ...
                tenant_id=configuration.control_tenant_id,
                actor_id=consumed.reviewer_actor_id,
                correlation_id=correlation,
                resource_type="user",
                ...
                outcome="success",
                metadata={
                    "requester_actor_id": consumed.requester_actor_id,
                    "requester_user_id": consumed.requester_user_id,
                    "approver_actor_id": consumed.reviewer_actor_id,
                    "approver_user_id": consumed.reviewer_user_id,
                    "created_user_id": result.user_id,
                    "created_actor_id": result.actor_id,
        ...
        raise
    except StagingBootstrapAuthorizationError as exc:
        safe_codes = {
