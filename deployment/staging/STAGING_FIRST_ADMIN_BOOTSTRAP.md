...
This is a one-time, operator-only bootstrap for exactly one `soc_manager` in the externally approved staging control tenant. It reuses `PrivilegedIdentityProvisioningService` and the existing `ApprovalWorkflow`. It does not create an admin, add a web route, or use public registration.
...
The approval must be created through `ApprovalWorkflow` by a verified requester and approved by a distinct verified reviewer. Its hashed artifact binds:
