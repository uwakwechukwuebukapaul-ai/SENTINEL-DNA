...
class ExecutivePostureSummary:
    ...
    threat_score: float = 0.0
    detection_posture: Any = None
    agent_activity: Any = None
    availability: str = "complete"
    def to_dict(self): return {key: _dict(value) for key, value in asdict(self).items()}
...
class SOCCommandSnapshot:
    ...
    investigations: list[InvestigationOverview] = field(default_factory=list)
    threat_posture: ThreatPostureView = field(default_factory=ThreatPostureView)
    executive_posture: ExecutivePostureSummary = field(default_factory=ExecutivePostureSummary)
    pending_decisions: list[DecisionQueueItem] = field(default_factory=list)
    availability: str = "complete"
