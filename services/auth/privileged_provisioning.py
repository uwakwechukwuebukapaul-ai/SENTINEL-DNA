...
def acquire_privileged_tenant_lock(connection: Any, tenant_id: str) -> None:
    ...
    if getattr(connection, "backend_name", "sqlite") == "postgresql":
        connection.execute(
            "SELECT tenant_id FROM canonical_tenants WHERE tenant_id=? FOR UPDATE",
            (str(tenant_id).strip(),),
        )
        connection.execute(
...
def acquire_privileged_tenant_transaction_lock(connection: Any, tenant_id: str) -> None:
    """Shared caller-owned PostgreSQL transaction lock for privileged races."""
    acquire_privileged_tenant_lock(connection, tenant_id)
...
__all__ = [
    ...
    "acquire_privileged_tenant_transaction_lock",
