...
def create_governed_billing_blueprint(application: BillingApplicationService, context_provider=None, webhook_tenant_provider=None):
    if not isinstance(application,BillingApplicationService) or not callable(context_provider): return None
    ...
    def checkout():
        ...
        try:
            result=application.create_checkout_request(context(),CheckoutRequest(payload["plan_id"],str(payload.get("email","")),str(payload.get("idempotency_key",""))))
    ...
    def status():
        try:
            value=application.get_billing_status(context()); return jsonify({"tenant_id":value.tenant_id,"subscription_status":value.subscription_status,"plan_id":value.plan_id,"entitlements":sorted(value.entitlement_capabilities),"transaction_reference":value.transaction_reference,"transaction_status":value.transaction_status})
        except Exception as exc: return error(exc)
