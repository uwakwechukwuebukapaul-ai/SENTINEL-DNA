...
def ensure_billing_schema(connection):
    ...
    CREATE TABLE IF NOT EXISTS crypto_verification_results (payment_intent_id TEXT PRIMARY KEY, provider TEXT NOT NULL, provider_payment_reference TEXT, asset TEXT NOT NULL, network TEXT NOT NULL, destination TEXT NOT NULL, observed_amount TEXT, expected_quoted_amount TEXT NOT NULL, confirmation_count INTEGER NOT NULL, required_confirmations INTEGER NOT NULL, verification_status TEXT NOT NULL, verified_at TEXT NOT NULL, expired INTEGER NOT NULL, reason_code TEXT NOT NULL);
    """)
...
class BillingRepository:
    ...
    def update_crypto_intent(self, connection, intent_id, status, result):
        connection.execute("UPDATE crypto_payment_intents SET status=?,updated_at=CURRENT_TIMESTAMP WHERE payment_intent_id=? AND status NOT IN ('SUCCESS','FAILED','CANCELLED')", (status, intent_id))
        connection.execute("INSERT INTO crypto_verification_results(payment_intent_id,provider,provider_payment_reference,asset,network,destination,observed_amount,expected_quoted_amount,confirmation_count,required_confirmations,verification_status,verified_at,expired,reason_code) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(payment_intent_id) DO UPDATE SET provider=excluded.provider,provider_payment_reference=excluded.provider_payment_reference,observed_amount=excluded.observed_amount,confirmation_count=excluded.confirmation_count,required_confirmations=excluded.required_confirmations,verification_status=excluded.verification_status,verified_at=excluded.verified_at,expired=excluded.expired,reason_code=excluded.reason_code", (result.payment_intent_id,result.provider,result.provider_payment_reference,result.asset,result.network,result.destination,None if result.observed_amount is None else str(result.observed_amount),str(result.expected_quoted_amount),result.confirmation_count,result.required_confirmations,result.verification_status,result.verified_at.isoformat(),int(result.expired),result.reason_code))
    def get_crypto_verification(self, connection, intent_id): return connection.execute("SELECT * FROM crypto_verification_results WHERE payment_intent_id=?", (intent_id,)).fetchone()
    def find_pending_crypto_intents(self, connection, tenant_id, limit):
        states=("AWAITING_PAYMENT","PAYMENT_DETECTED","CONFIRMING")
        if tenant_id: return connection.execute("SELECT * FROM crypto_payment_intents WHERE tenant_id=? AND status IN (?,?,?) ORDER BY payment_expires_at ASC,payment_intent_id ASC LIMIT ?", (tenant_id,*states,limit)).fetchall()
        return connection.execute("SELECT * FROM crypto_payment_intents WHERE status IN (?,?,?) ORDER BY payment_expires_at ASC,payment_intent_id ASC LIMIT ?", (*states,limit)).fetchall()
