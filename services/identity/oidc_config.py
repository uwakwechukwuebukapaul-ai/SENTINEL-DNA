...
class OidcRuntimeConfiguration:
    provider: str; issuer: str; authorization_endpoint: str; token_endpoint: str; jwks_uri: str; client_id: str; audience: str; redirect_uri: str; client_secret_reference: str; provider_tenant_claim: str; signing_algorithms: tuple[str, ...]
    @classmethod
    def from_environment(cls, environ=None):
        env = environ or os.environ
        algorithms = tuple(x.strip() for x in env.get("OIDC_SIGNING_ALGORITHMS", "").split(",") if x.strip())
        return cls(*(env.get(key, "").strip() for key in ("OIDC_PROVIDER", "OIDC_ISSUER", "OIDC_AUTHORIZATION_ENDPOINT", "OIDC_TOKEN_ENDPOINT", "OIDC_JWKS_URI", "OIDC_CLIENT_ID", "OIDC_AUDIENCE", "OIDC_REDIRECT_URI", "OIDC_CLIENT_SECRET_REFERENCE", "OIDC_PROVIDER_TENANT_CLAIM")), algorithms)
