...
def _evidence_inputs(tmp_path: Path) -> tuple[dict[str, str], Path, Path]:
    ...
    # The workspace intentionally contains this uncommitted implementation
    # while its tests run.  Exercise the existing manifest builder's behavior
    # without weakening the validator itself.
    with patch("deployment.scripts.release_manifest._assert_clean_worktree"), patch(
        ...
        manifest = build_manifest(repository_root=ROOT, image_digest=digest, image_id="sha256:image-id")
    release_manifest = tmp_path / "release-manifest.json"
    with patch("deployment.scripts.release_manifest._assert_outside_repository"):
        write_manifest(manifest, output=release_manifest, repository_root=ROOT)
    return environment, release_manifest, trusted
...
def test_all_contracts_pass_with_local_nonsecret_evidence(tmp_path: Path):
    environment, release_manifest, _ = _evidence_inputs(tmp_path)
...
def test_report_writer_is_outside_repo_and_append_only(tmp_path: Path):
    ...
    with patch("deployment.scripts.release_manifest._assert_clean_worktree"), patch(
        ...
        report = DeploymentContractValidator(repository_root=ROOT, environ=environment, release_manifest=release_manifest).run()
    ...
    with pytest.raises(ValueError, match="outside_repository"):
        write_immutable_report(report, ROOT / "evidence.json", repository_root=ROOT)
